...
$("#open").click(function () {
  ...
  //ANALYZER 2D ##########################################################################################
  var analyser0 = new ambisonics.intensityAnalyser(context); //Setter opp analyser for hver enkelt lyd
  var analyser1 = new ambisonics.intensityAnalyser(context);
  ...
  var analyserMono = new ambisonics.intensityAnalyser(context);
  console.log(analyser0);
  //FuMa (b-format rekkefølge til ACN <--> WXYZ til WYZX) ################################################
  var converterF2A_0 = new ambisonics.converters.wxyz2acn(context); //konverterer fra b-format til ACN
  var converterF2A_1 = new ambisonics.converters.wxyz2acn(context);
  ...
  converterF2A_0.out.connect(rotator.in);
  converterF2A_1.out.connect(rotator.in);
  ...
  converterF2A_0.out.connect(analyser0.in);
  converterF2A_1.out.connect(analyser1.in);
  ...
  monoEncoder.out.connect(analyserMono.in);
  ...
  function changeAudio(lydfil) {
    ...
    if (document.getElementById("audioElement" + select.value) == null) {
      ...
      if (select.value == 0) {
        mediaElementSource.connect(converterF2A_0.in); //Kobler til signalkjede
        var node = document.createTextNode("Test"); //Gir tekst til audio-elementet
        label.appendChild(node);
        divElement.appendChild(label);
      } else if (select.value == 1) {
        mediaElementSource.connect(converterF2A_1.in);
        var node = document.createTextNode("Hav");
        label.appendChild(node);
        divElement.appendChild(label);
  ...
  function updateVisual() {
    ...
    analyser0.updateBuffers();
    analyser1.updateBuffers();
    analyser2.updateBuffers();
    analyser3.updateBuffers();
    analyser4.updateBuffers();
    analyser5.updateBuffers();
    analyserMono.updateBuffers();

    var params0 = analyser0.computeIntensity(); //setter opp en analyser for hver lydfil og legger de i variabler som sendes til A-frame
    var params1 = analyser1.computeIntensity();
    ...
    var paramsMono = analyserMono.computeIntensity();
  ...
  function update3d(params) {
    ...
    document.getElementById("OverlayPitch").innerHTML =
      "X: " + camPitch.toFixed(1);

    document.getElementById("OverlayYaw").innerHTML =
      "Yaw: " + camYaw.toFixed(1);

    document.getElementById("OverlayRoll").innerHTML =
      "Roll: " + camRoll.toFixed(1);
